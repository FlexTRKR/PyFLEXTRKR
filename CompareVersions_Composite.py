...
def Plot_CloudTracks(zipped_inputs):
    ...
    ax0 = fig.add_axes([0.05, 0.25, 0.25, 0.5])
    ax1 = fig.add_axes([0.38, 0.25, 0.25, 0.5])
    ...
    im0 = map0.pcolormesh(Longitude, Latitude, np.ma.masked_invalid(np.atleast_2d(BrightnessTemperature)), cmap='gist_stern', vmin=200, vmax=300)
    ...
    im1 = map1.pcolormesh(Longitude, Latitude, np.ma.masked_invalid(np.atleast_2d(IDLCloudTracks)), cmap='nipy_spectral_r', vmin=MinIDLTracks, vmax=MaxIDLTracks, linewidth=0)
    cbar1 = plt.colorbar(im1, ax=ax1, fraction=0.03, pad=0.04)
    cbar1.ax.set_xlabel('#', fontsize=8, labelpad=10)
    ax1.set_xlabel('Tracks Present: ' + str(np.unique(IDLCloudTracks)[1::]), fontsize=8, labelpad=20)
    ...
    im2 = map2.pcolormesh(Longitude, Latitude, np.ma.masked_invalid(np.atleast_2d(PythonCloudTracks)), cmap='nipy_spectral_r', vmin=MinPythonTracks, vmax=MaxPythonTracks, linewidth=0)
    cbar2 = plt.colorbar(im2, ax=ax2, fraction=0.03, pad=0.04)
    cbar2.ax.set_xlabel('#', fontsize=8, labelpad=10)
    ax2.set_xlabel('Tracks Present: ' + str(np.unique(PythonCloudTracks)[1::]), fontsize=8, labelpad=20)
