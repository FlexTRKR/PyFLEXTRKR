...
def calc_stats_radar(tracknumbers, cloudidfiles, tracking_inpath, cloudid_filebase, 
    ...
    if np.nanmax(file_tracknumbers) > 0:
        ...
        for itrack in range(numtracks):
            ...
            cloudnumber = np.array(np.where(file_tracknumbers == uniquetracknumbers[itrack]))[0, :] + 1
            ...
            if len(cloudnumber) == 1:
                ...
                if (ncellpix > 0):
                    ...
                    finaltrack_cell_maxdbz[itrack] = np.nanmean(file_dbz[0,cellarea[0],cellarea[1]])

                    # Echo-top heights
                    finaltrack_cell_maxETH10dbz[itrack] = np.nanmax(echotop10[0,cellarea[0],cellarea[1]])
                    finaltrack_cell_maxETH20dbz[itrack] = np.nanmax(echotop20[0,cellarea[0],cellarea[1]])
                    finaltrack_cell_maxETH30dbz[itrack] = np.nanmax(echotop30[0,cellarea[0],cellarea[1]])
                    finaltrack_cell_maxETH40dbz[itrack] = np.nanmax(echotop40[0,cellarea[0],cellarea[1]])
                    finaltrack_cell_maxETH50dbz[itrack] = np.nanmax(echotop50[0,cellarea[0],cellarea[1]])
        ...
        return uniquetracknumbers, numtracks, finaltrack_basetime, finaltrack_cloudnumber, finaltrack_cloudidfile, \
                finaltrack_core_meanlat, finaltrack_core_meanlon, finaltrack_core_mean_x, finaltrack_core_mean_y, \
                finaltrack_cell_meanlat, finaltrack_cell_meanlon, finaltrack_cell_mean_x, finaltrack_cell_mean_y, \
                finaltrack_cell_minlat, finaltrack_cell_maxlat, finaltrack_cell_minlon, finaltrack_cell_maxlon, \
                finaltrack_cell_min_y, finaltrack_cell_max_y, finaltrack_cell_min_x, finaltrack_cell_max_x, \
                finaltrack_dilatecell_meanlat, finaltrack_dilatecell_meanlon, finaltrack_dilatecell_mean_x, finaltrack_dilatecell_mean_y, \
