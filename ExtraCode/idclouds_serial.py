...
def idclouds_mergedir(datasource, datadescription, datapath, databasename, fileyearindices, filemonthindices, filedayindices, filehourindices, fileminuteindices, cloudid_version, dataoutpath, latlon_file, geolimits, startdate, enddate, pixel_radius, area_thresh, cloudtb_threshs, absolutetb_threshs, miss_thresh, warmanvilexpansion):
    # inputs:
    ...
    if datasource == 'mergedir':
        ...
        # loop through all the files in the directory. only load data from files within start and end time span. 
        for file in os.listdir(datapath):
            if file.endswith(".nc") and file[fileyearindices[0]:filedayindices[1]]>=startdate and file[fileyearindices[0]:filedayindices[1]]<=enddate:    # isolate cases within time period set by startdate and enddate
                # isolate date and time information
                file_year = int(file[fileyearindices[0]:fileyearindices[1]])
                file_month = int(file[filemonthindices[0]:filemonthindices[1]])
                file_day = int(file[filedayindices[0]:filedayindices[1]])
                file_hour = int(file[filehourindices[0]:filehourindices[1]])
                file_minute = int(file[fileminuteindices[0]:fileminuteindices[1]])
                file_second = 0
                file_date = int(file[fileyearindices[0]:filedayindices[1]])
                file_time = int(file[filehourindices[0]:fileminuteindices[1]])

                file_basetime = datetime.datetime(file_year, file_month, file_day, file_hour, file_minute, file_second)
                file_basetime = calendar.timegm(file_basetime.timetuple())

                datetimestring = str(int(file_year)) + str(int(file_month)).zfill(2) + str(int(file_day)).zfill(2) + '_' + str(int(file_hour)).zfill(2) + str(int(file_minute)).zfill(2)

                # load brighttness temperature data. automatically removes missing values
                rawdata = Dataset(datapath + file, 'r')           # open file
                ...
                if len(lat_indices) > 0 and len(lon_indices) > 0:
                    ...
                    if np.divide(missingcount, (ny*nx)) < miss_thresh:
                        ...
                        # create file
                        filesave = Dataset(dataoutpath + datasource + '_' + datadescription + '_cloudid' + cloudid_version + '_' + datetimestring + '.nc', 'w', format='NETCDF4_CLASSIC')

                        # set global attributes
                        filesave.Convenctions = 'CF-1.6'
                        filesave.title = 'Statistics about convective features identified in the data from ' + str(int(file_year)) + '/' + str(int(file_month)).zfill(2) + '/' + str(int(file_day)).zfill(2) + ' ' + str(int(file_hour)).zfill(2) + ':' + str(int(file_minute)).zfill(2) + ' utc'
                        ...
                        basetime[:] = file_basetime
                        filedate[0,:] = stringtochar(np.array(datetimestring[0:8]))
                        filetime[0,:] =  stringtochar(np.array(datetimestring[9::]))
