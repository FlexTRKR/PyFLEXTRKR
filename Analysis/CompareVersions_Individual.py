...
ax0.set_xmargin(0)
ax0.set_ymargin(0)
im0 = ax0.pcolormesh(Longitude, Latitude, np.ma.masked_invalid(np.atleast_2d(IDLCloudTracks)), cmap='nipy_spectral_r', vmin=int(sys.argv[3])-1, vmax=int(sys.argv[4]), linewidth=0, transform=ccrs.PlateCarree())
...
ax1.set_xmargin(0)
ax1.set_ymargin(0)
im1 = ax1.pcolormesh(Longitude, Latitude, np.ma.masked_invalid(np.atleast_2d(PythonCloudTracks)), cmap='nipy_spectral_r', vmin=int(sys.argv[5])-1, vmax=int(sys.argv[6]), linewidth=0, transform=ccrs.PlateCarree())
