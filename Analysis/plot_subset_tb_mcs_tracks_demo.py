"""
Demonstrates ploting MCS tracks on Tb snapshots for a subset domain.

>python plot_subset_tb_mcs_tracks_demo.py -s STARTDATE -e ENDDATE -c CONFIG.yml -o horizontal 
Optional arguments:
...
def plot_map_2panels(pixel_dict, plot_info, map_info, track_dict):
    ...
    tn1 = ax1.pcolormesh(lon, lat, Tn, vmin=min(levels['tn_levels']), vmax=max(levels['tn_levels']),
                         cmap=cmap, transform=data_proj, zorder=2, alpha=1)  
...
if __name__ == "__main__":
    ...
    levels = {'tb_levels': tb_levels}
    # Colorbar ticks & labels
    cbticks = {'tb_ticks': tb_ticks, }
    cblabels = {'tb_label': 'T$_{b}$ (K)'}
    # Colormap
    tb_cmap = truncate_colormap(plt.get_cmap('turbo'), minval=0.05, maxval=0.95)
    # tn_cmap = cc.cm["glasbey_light"]
    tn_cmap = cc.cm["glasbey"]
    cmaps = {'tb_cmap': tb_cmap, 'tn_cmap': tn_cmap}
    titles = {'tb_title': 'IR Brightness Temperature (Tracked MCS Outline)'}
    plot_info = {
        'levels': levels,
        'cmaps': cmaps,
        'titles': titles,
    ...
    if trackstats_file is None:
        trackstats_file = f"{stats_path}{mcstbstats_filebase}{startdate}_{enddate}.nc"
    if pixeltracking_path is None:
        pixeltracking_path = f"{config['root_path']}mcstracking_tb/{startdate}_{enddate}/"
