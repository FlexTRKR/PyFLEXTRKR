...
# Maximum cold cloud shield
IDL_maxcoldanvilarea = np.nanmax(IDL_coldanvilarea, axis=1)
hIDL_maxcoldanvilarea, bins_maxcoldanvilarea = np.histogram(IDL_maxcoldanvilarea, bins=np.arange(0, 1200, 50))

Python_maxcoldanvilarea = np.nanmax(Python_coldanvilarea, axis=1)
hPython_maxcoldanvilarea, bins_maxcoldanvilarea = np.histogram(Python_maxcoldanvilarea, bins=np.arange(0, 1200, 50))

bins_maxcoldanvilarea = np.divide((bins_maxcoldanvilarea[0:-1] + bins_maxcoldanvilarea[1::]), 2)

plt.figure()
plt.title('Maximum Cold Anvil Area', fontsize=14, y=1.01)
plt.plot(bins_maxcoldanvilarea, hIDL_maxcoldanvilarea, color='tomato', linewidth=3)
plt.plot(bins_maxcoldanvilarea, hPython_maxcoldanvilarea, color='dodgerblue', linewidth=3)
plt.legend(['IDL (mean: ' + str(np.round(np.nanmean(IDL_maxcoldanvilarea), 2)) + ' km)', 'Python (mean: ' + str(np.round(np.nanmean(Python_maxcoldanvilarea), 2)) + ' km)'], fontsize=12)
plt.xlim(50, 950)
...
plt.tick_params(labelsize=10)
plt.grid(True, linestyle=':')
plt.savefig(Figure_Location + 'IDLPythonComparison_Season_ColdAnvilAreaPDF.png')
plt.close()
