...
-o, --orientation     Panel orientation: 'vertical' (left+right panel) or 'horizontal' (top+bottom panel)
...
def parse_cmd_args():
    ...
    parser.add_argument("-o", "--orientation", help="panel orientation ('vertical' (left+right panel) or 'horizontal' (top+bottom panel))", required=True)
...
def plot_map_2panels(pixel_dict, plot_info, map_info, track_dict):
    ...
    Args:
        ...
        plot_info: dictionary
            Dictionary containing plotting setup variables:
                ...
                titles: dict with 'tb_title', 'pcp_title'
                ...
                panel_orientation: 'vertical' or 'horizontal'
        map_info: dictionary
    ...
    for itrack in range(0, ntracks):
        ...
        if (idt_match < dt_match):
            ...
            if ~np.isnan(_irad) and ~np.isnan(_ilon) and ~np.isnan(_ilat):
...
def work_for_time_loop(datafile, track_dict, map_info, plot_info, config):
    ...
    if subset == 1:
        ...
        if len(rows) > 0:
            ...
            spatial_dims = [d for d in ds[tb_varname].dims if d not in ['time', 'times']]
            
            if len(spatial_dims) >= 2:
                dim_y, dim_x = spatial_dims[-2], spatial_dims[-1]
                
                # Subset using index slicing - maintains rectangular structure, no NaNs
                tb_sub = ds[tb_varname].isel({dim_y: slice(row_min, row_max), 
                                               dim_x: slice(col_min, col_max)}).squeeze()
                dbz_sub = ds[dbz_varname].isel({dim_y: slice(row_min, row_max),
                                                 dim_x: slice(col_min, col_max)}).squeeze()
...
if __name__ == "__main__":
    ...
    if dbz_varname == 'reflectivity_comp':
        title_b = '(b) Composite Reflectivity (Tracked MCSs Shaded)'
    elif dbz_varname == 'reflectivity_lowlevel':
        title_b = f'(b) Low-level Reflectivity (Tracked MCSs Shaded)'
    titles = {'tb_title': '(a) IR Brightness Temperature', 'pcp_title': title_b}
    plot_info = {
        'tb_varname': 'tb',
