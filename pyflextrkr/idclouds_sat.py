...
def idclouds_gpmmergir(datafilepath, datasource, datadescription, variablename, cloudid_version, \
                        dataoutpath, latlon_file, geolimits, startdate, enddate, \
                        pixel_radius, area_thresh, cloudtb_threshs, absolutetb_threshs, miss_thresh, \
                        cloudidmethod, mincoldcorepix, smoothsize, warmanvilexpansion, \
                        idclouds_hourly, idclouds_minute, \
                        linkpf, pf_smooth_window, pf_dbz_thresh, pf_link_area_thresh, pfvarname, **kwargs):
    """
                    ...
                    if np.divide(missingcount, (ny*nx)) < miss_thresh:
                        ...
                        if (linkpf == 1):
                            ...
                            if (final_nclouds > 0):
                                ...
                                final_pcp = np.full(final_convcold_cloudnumber.shape, np.nan, dtype=float)
                                final_pf_number = np.full(final_convcold_cloudnumber.shape, 0, dtype=int)
                                # Make a copy of the original arrays
                                final_cloudnumber_orig = final_cloudnumber
                                final_convcold_cloudnumber_orig = final_convcold_cloudnumber
                        else:
                        ...
                        if final_nclouds > 0:
                            ...
                            net.write_cloudid_wrf(cloudid_outfile, file_basetime, file_datestring, file_timestring, \
                                                ...
                                                precipitation=final_pcp, pf_number=final_pf_number, convcold_cloudnumber_orig=final_convcold_cloudnumber_orig, cloudnumber_orig=final_cloudnumber_orig, \
