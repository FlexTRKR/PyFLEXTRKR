...
def calc_stats_singlefile(
    ...
    if np.nanmax(tracknumbers) > 0:
        ...
        out_meanlat = np.full(numtracks, fillval_f, dtype=np.float)
        out_meanlon = np.full(numtracks, fillval_f, dtype=np.float)
        out_area = np.full(numtracks, fillval_f, dtype=np.float)
        ...
        if feature_type == "radar_cells":
            ...
            out_core_mean_y = np.full(numtracks, fillval_f, dtype=np.float)
            out_cell_meanlat = np.full(numtracks, fillval_f, dtype=np.float)
            out_cell_meanlon = np.full(numtracks, fillval_f, dtype=np.float)
            out_cell_mean_x = np.full(numtracks, fillval_f, dtype=np.float)
            out_cell_mean_y = np.full(numtracks, fillval_f, dtype=np.float)
            out_cell_max_dbz = np.full(numtracks, fillval_f, dtype=np.float)
            out_cell_maxETH10dbz = np.full(numtracks, fillval_f, dtype=np.float)
            out_cell_maxETH20dbz = np.full(numtracks, fillval_f, dtype=np.float)
            out_cell_maxETH30dbz = np.full(numtracks, fillval_f, dtype=np.float)
            out_cell_maxETH40dbz = np.full(numtracks, fillval_f, dtype=np.float)
            out_cell_maxETH50dbz = np.full(numtracks, fillval_f, dtype=np.float)
