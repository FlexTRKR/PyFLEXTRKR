...
def define_robust_mcs_pf(stats_path, pfstats_filebase, startdate, enddate, timeresolution, \
                        geolimits, mcs_pf_majoraxisthresh, mcs_pf_durationthresh, aspectratiothresh, \
                        lifecyclethresh, lengththresh, gapthresh, \
    ...
    for nt in range(0, ntracks):
        ...
        ipfmcs = np.array(np.where((ipf_majoraxis > mcs_pf_majoraxisthresh) & (ipf_majoraxis < max_pf_majoraxis_thresh)))[0, :]
        nipfmcs = len(ipfmcs)
        # print(nipfmcs)
        ...
        if nipfmcs > 0 :
            ...
            if nipfmcs*time_res > mcs_pf_durationthresh:
                ...
                for igroup in range(0, nbreaks):
                    ...
                    igroup_duration = np.multiply((groups[igroup][-1] - groups[igroup][0] + 1), time_res)
                    ...
                    mcs_heavyratio = coefs_heavyratio[0] + coefs_heavyratio[1] * igroup_duration
                    ...
                    if igroup_duration >= mcs_pf_durationthresh:   # KB CHANGED
                        ...
                        ct_pftimes = np.count_nonzero((igroup_pfarea > mcs_pfarea) & (igroup_pfrate > mcs_rravg) & (igroup_pfskew > mcs_rrskew))
                        dur_pf = float(ct_pftimes) * time_res

                        # Calculate volumetric heavy rain ratio during this sub-period
                        heavyrain_ratio = 100 * np.nansum(igroup_volrainheavy) / np.nansum(igroup_volrainall)

                        # Duration of PF satisfying MCS criteria >= pf_mcs_dur [hour] and 
                        # heavy rain ratio during the sub-period >= mcs_heavyratio
                        if (dur_pf >= mcs_pf_durationthresh) & (heavyrain_ratio > mcs_heavyratio):
...
#                 cycle_index[ilongmcs[ilm], :] = np.copy(ilm_index)
    ...
    statistics_outfile = stats_path + 'robust_mcs_tracks_' + data.attrs['startdate'] + '_' + data.attrs['enddate'] + '.nc'

    # Define xarrray dataset
    output_data = xr.Dataset({'mcs_length': (['tracks'], data['length'][trackid_mcs]), \
                              ...
                              'interruptions': (['tracks'], data['interruptions'][trackid_mcs]), \
                            #   'boundary': (['tracks'], data['boundary'][trackid_mcs]), \
                              'base_time': (['tracks', 'times'], mcs_basetime), \
                              'datetimestring': (['tracks', 'times', 'characters'], data['datetimestring'][trackid_mcs, :, :]), \
                              ...
                              'mergecloudnumber': (['tracks', 'times', 'mergesplit'], data['mergecloudnumber'][trackid_mcs, :, :]), \
                              'splitcloudnumber': (['tracks', 'times', 'mergesplit'], data['splitcloudnumber'][trackid_mcs, :, :]), \
                            ...
                            #   'pf_frac': (['tracks', 'times'], data['pf_frac'][trackid_mcs]), \
                              ...
                              'pf_rainrate': (['tracks', 'times', 'pfs'], data['pf_rainrate'][trackid_mcs, :, :]), \
                              'pf_skewness': (['tracks', 'times', 'pfs'], data['pf_skewness'][trackid_mcs, :, :]), \
                              'pf_majoraxislength': (['tracks', 'times', 'pfs'], data['pf_majoraxislength'][trackid_mcs, :, :]), \
                              'pf_minoraxislength': (['tracks', 'times', 'pfs'], data['pf_minoraxislength'][trackid_mcs, :, :]), \
                              'pf_aspectratio': (['tracks', 'times', 'pfs'], data['pf_aspectratio'][trackid_mcs, :, :]), \
                              'pf_eccentricity': (['tracks', 'times', 'pfs'], data['pf_eccentricity'][trackid_mcs, :, :]), \
                              'pf_orientation': (['tracks', 'times', 'pfs'], data['pf_orientation'][trackid_mcs, :, :])}, \
                             coords={'tracks': (['tracks'], np.arange(1, len(trackid_mcs)+1)), \
    ...
    output_data['times'].attrs['description'] = 'Maximum number of features in a given track'
