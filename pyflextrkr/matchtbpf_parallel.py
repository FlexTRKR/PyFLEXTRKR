...
def match_tbpf_tracks(mcsstats_filebase, cloudid_filebase, stats_path, cloudidtrack_path, \
                        startdate, enddate, geolimits, nmaxpf, nmaxcore, nmaxclouds, pf_rr_thres, pixel_radius, irdatasource, precipdatasource, \
                        ...
                        landmask_file=None, landvarname=None, landfrac_thresh=None):
    ...
    pf_accumrainheavy = np.ones((ir_ntracks, ir_nmaxlength, nmaxpf), dtype=float)*np.nan
    ...
    statistics_outfile = stats_path + 'mcs_tracks_pf_' + startdate + '_' + enddate + '.nc'
    ...
    with Pool(nprocesses) as pool:
        results = pool.starmap(matchtbpf_singlefile,[(filename, cloudidtrack_path, cloudid_filebase, \
                    ...
                    landmask_file, landvarname, landfrac_thresh) for filename in cloudidfilelist])
        pool.close()
    ...
    for ifile in range(nfiles):
        ...
        if (tmp is not None):
            ...
            for imatch in range(nmatchpftmp):
                ...
                pf_maxrainrate[matchindicestmp[0,imatch],matchindicestmp[1,imatch],:] = tmp[17][imatch,:]
                pf_accumrain[matchindicestmp[0,imatch],matchindicestmp[1,imatch],:] = tmp[18][imatch,:]
                pf_accumrainheavy[matchindicestmp[0,imatch],matchindicestmp[1,imatch],:] = tmp[19][imatch,:]
                pf_landfrac[matchindicestmp[0,imatch],matchindicestmp[1,imatch]] = tmp[20][imatch]
                total_rain[matchindicestmp[0,imatch],matchindicestmp[1,imatch]] = tmp[21][imatch]
                total_heavyrain[matchindicestmp[0,imatch],matchindicestmp[1,imatch]] = tmp[22][imatch]
                rainrate_heavyrain[matchindicestmp[0,imatch],matchindicestmp[1,imatch]] = tmp[23][imatch]
                basetime[matchindicestmp[0,imatch],matchindicestmp[1,imatch]] = tmp[24][imatch]
                precip_basetime[matchindicestmp[0,imatch],matchindicestmp[1,imatch]] = tmp[25][imatch]
                # import pdb; pdb.set_trace()
    ...
    output_data = xr.Dataset({'mcs_length':(['tracks'], np.squeeze(ir_mcslength)), \
                              ...
                              'mergecloudnumber': (['tracks', 'times', 'mergesplit'], ir_mergecloudnumber), \
                              'splitcloudnumber': (['tracks', 'times', 'mergesplit'], ir_splitcloudnumber), \
                             ...
                             coords={'tracks': (['tracks'], np.arange(1, ir_ntracks+1)), \
    ...
    output_data.status.attrs['values'] = f'{missing_val}=missing cloud or cloud removed due to short track, '+\
    ...
    output_data.startstatus.attrs['values'] = f'{missing_val}=missing cloud or cloud removed due to short track, '+\
                                        '0=track ends here, 1=cloud continues as one cloud in next file, '+\
                                        '2=Biggest cloud in merger, 21=Smaller cloud(s) in merger, 13=Cloud that splits, '+\
                                        '3=Biggest cloud from a split that stops after the split, '+\
                                        '31=Smaller cloud(s) from a split that stop after the split. '+\
    ...
    output_data.endstatus.attrs['values'] = f'{missing_val}=missing cloud or cloud removed due to short track,  '+\
                                        '0=track ends here, 1=cloud continues as one cloud in next file,  '+\
                                        '2=Biggest cloud in merger, 21=Smaller cloud(s) in merger, 13=Cloud that splits,  '+\
                                        '3=Biggest cloud from a split that stops after the split,  '+\
                                        '31=Smaller cloud(s) from a split that stop after the split.  '+\
    ...
    output_data.interruptions.attrs['values'] = '0 = full track available, good data.  '+\
    ...
    output_data.base_time.attrs['long_name'] = 'epoch time (seconds since 01/01/1970 00:00) of file'
    ...
    output_data.cloudnumber.attrs['usage'] = 'to link this tracking statistics file with pixel-level cloudid files,  '+\
    ...
    output_data.mergecloudnumber.attrs['usage'] = 'to link this tracking statistics file with pixel-level cloudid files,  '+\
        'use the cloudidfile and cloudnumber together to identify which cloud this current track and time is associated with'
    ...
    output_data.splitcloudnumber.attrs['usage'] = 'to link this tracking statistics file with pixel-level cloudid files, '+\
        'use the cloudidfile and cloudnumber together to identify which cloud this current track and time is associated with'
    ...
    output_data.pf_rainrate.attrs['long_name'] = 'mean precipitation rate of each precipitation feature at a given time'
    ...
    output_data.pf_maxrainrate.attrs['long_name'] = 'max precipitation rate of each precipitation feature at a given time'
    output_data.pf_maxrainrate.attrs['units'] = 'mm/hr'

    output_data.pf_accumrain.attrs['long_name'] = 'accumulated precipitation of each precipitation feature at a given time'
    output_data.pf_accumrain.attrs['units'] = 'mm/hr'

    output_data.pf_accumrainheavy.attrs['long_name'] = 'accumulated heavy precipitation of each precipitation feature at a given time'
    output_data.pf_accumrainheavy.attrs['units'] = 'mm/hr'
    output_data.pf_accumrainheavy.attrs['heavy_rainrate_threshold'] = heavy_rainrate_thresh

    output_data.pf_landfrac.attrs['long_name'] = 'Fraction of precipitation feature over land'
    output_data.pf_landfrac.attrs['units'] = 'fraction'
    
    output_data.total_rain.attrs['long_name'] = 'total precipitation under cold cloud shield (all rainfall included)'
    output_data.total_rain.attrs['units'] = 'mm/h'

    output_data.total_heavyrain.attrs['long_name'] = f'total heavy precipitation under cold cloud shield'
    output_data.total_heavyrain.attrs['units'] = 'mm/h'
    output_data.total_heavyrain.attrs['heavy_rainrate_threshold'] = heavy_rainrate_thresh

    output_data.rainrate_heavyrain.attrs['long_name'] = 'mean heavy precipitation rate under cold cloud shield'
    output_data.rainrate_heavyrain.attrs['units'] = 'mm/h'
    output_data.rainrate_heavyrain.attrs['heavy_rainrate_threshold'] = heavy_rainrate_thresh
    
    # Write netcdf file
    print('')
    print(statistics_outfile)
    output_data.to_netcdf(path=statistics_outfile, mode='w', format='NETCDF4_CLASSIC', unlimited_dims='tracks', \
                          encoding={'mcs_length': {'dtype': 'int', 'zlib':True, '_FillValue': missing_val}, \
                                    ...
                                    'mergecloudnumber': {'dtype': 'int', 'zlib':True, '_FillValue': missing_val}, \
                                    'splitcloudnumber': {'dtype': 'int', 'zlib':True, '_FillValue': missing_val}, \
                                    ...
                                    'pf_majoraxislength': {'dtype': 'float32', 'zlib':True, '_FillValue': np.nan}, \
                                    'pf_minoraxislength': {'dtype': 'float32', 'zlib':True, '_FillValue': np.nan}, \
                                    ...
                                    'pf_accumrainheavy': {'dtype': 'float32', 'zlib':True, '_FillValue': np.nan}, \
                                    ...
                                    'rainrate_heavyrain': {'dtype': 'float32', 'zlib':True, '_FillValue': np.nan}, \
                                    })
