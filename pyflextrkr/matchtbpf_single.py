def matchtbpf_singlefile(cloudid_filename, cloudidtrack_path, cloudid_filebase, \
                        ir_basetime, ir_cloudnumber, ir_mergecloudnumber, ir_splitcloudnumber, \
                        rr_min, pf_link_area_thresh, heavy_rainrate_thresh, pixel_radius, nmaxpf, precipdatasource, \
    ...
    if os.path.isfile(cloudid_filename):
        ...
        if (nmatchcloud > 0):
            ...
            for imatchcloud in range(nmatchcloud):
                ...
                if ncloudpix > 0:
                    ...
                    if len(idx_heavyrain[0]) > 0:
                        total_heavyrain[imatchcloud] = np.nansum(subrainratemap[idx_heavyrain])
                        rainrate_heavyrain[imatchcloud] = np.nanmean(subrainratemap[idx_heavyrain])
                    ...
                    if nrainpix > 0:                                              
                        ...
                        if landmask is not None:
                            ...
                            if precipdatasource == 'imerg':
                                ...
                                print('Error, unknown precipdatasource: ' + precipdatasource)
                                print('Must define how to calculate landfrac.')
                        ...
                        if numpf > 0:
                           ...
                           # Initialize matrices
                            ...
                            for ipf in range(1, npf_save+1):
                                ...
                                iipfy_heavy, iipfx_heavy = np.array(np.where((pfnumberlabelmap == ipf) & (subrainratemap > heavy_rainrate_thresh)))
                                ...
                                if (niipfpix != pf_npix[ipf -1]):
                                    ...
                                    pfmaxrainrate[ipf-1] = np.nanmax(subrainratemap[iipfy[:], iipfx[:]])
                                    ...
                                    pfaccumrain[ipf-1] = np.nansum(subrainratemap[iipfy[:], iipfx[:]])
                                    if niipfpix_heavy > 0:
                                        pfaccumrainheavy[ipf-1] = np.nansum(subrainratemap[iipfy_heavy[:], iipfx_heavy[:]])
                            ...
                            pf_maxrainrate[imatchcloud, 0:npf_save] = pfmaxrainrate[0:npf_save]
                            ...
                            pf_accumrain[imatchcloud, 0:npf_save] = pfaccumrain[0:npf_save]
                            pf_accumrainheavy[imatchcloud, 0:npf_save] = pfaccumrainheavy[0:npf_save]
            ...
            return nmatchcloud, matchindices, pf_npf, pf_pflon, pf_pflat, pf_pfnpix, pf_pfrainrate, pf_pfskewness, pf_pfmajoraxis, \
                    pf_pfminoraxis, pf_pfaspectratio, pf_pforientation, pf_pfeccentricity, \
                    pf_pfycentroid, pf_pfxcentroid, pf_pfyweightedcentroid, pf_pfxweightedcentroid, \
                    pf_maxrainrate, pf_accumrain, pf_accumrainheavy, pf_landfrac, total_rain, total_heavyrain, rainrate_heavyrain, \
                    basetime, precip_basetime
