...
def trackstats_radar(
    ...
    for nf in range(0, nfiles):
        ...
        if np.nanmax(file_tracknumbers) > 0:
            ...
            echotop10 = (
                file_cloudiddata["echotop10"][:] / 1000.0
            )  # convert unit to [km]
            echotop20 = (
                file_cloudiddata["echotop20"][:] / 1000.0
            )  # convert unit to [km]
            echotop30 = (
                file_cloudiddata["echotop30"][:] / 1000.0
            )  # convert unit to [km]
            echotop40 = (
                file_cloudiddata["echotop40"][:] / 1000.0
            )  # convert unit to [km]
            echotop50 = (
                file_cloudiddata["echotop50"][:] / 1000.0
            )  # convert unit to [km]
                ...
                if len(cloudnumber) == 1:
                    ...
                    if nc < maxtracklength:
                        ...
                        if ncellpix > 0:
                            ...
                            dilatecelllat = latitude[
                                dilatecellarea[0], dilatecellarea[1]
                            ]
                            dilatecelllon = longitude[
                                dilatecellarea[0], dilatecellarea[1]
                            ]
                            ...
                            finaltrack_core_meanlat[itrack - 1, nc] = np.nanmean(
                                corelat
                            )
                            finaltrack_core_meanlon[itrack - 1, nc] = np.nanmean(
                                corelon
                            )
                            ...
                            finaltrack_cell_meanlat[itrack - 1, nc] = np.nanmean(
                                celllat
                            )
                            finaltrack_cell_meanlon[itrack - 1, nc] = np.nanmean(
                                celllon
                            )
                            ...
                            finaltrack_dilatecell_meanlat[itrack - 1, nc] = np.nanmean(
                                dilatecelllat
                            )
                            finaltrack_dilatecell_meanlon[itrack - 1, nc] = np.nanmean(
                                dilatecelllon
                            )
                            finaltrack_dilatecell_mean_y[itrack - 1, nc] = np.nanmean(
                                dilatecell_y
                            )
                            finaltrack_dilatecell_mean_x[itrack - 1, nc] = np.nanmean(
                                dilatecell_x
                            )
                            ...
                            # Area of the cell
                            finaltrack_core_area[itrack - 1, nc] = (
                                ncorepix * pixel_radius ** 2
                            )
                            finaltrack_cell_area[itrack - 1, nc] = (
                                ncellpix * pixel_radius ** 2
                            )
                            ...
                            # Echo-top heights
                            finaltrack_cell_maxETH10dbz[itrack - 1, nc] = np.nanmax(
                                echotop10[0, cellarea[0], cellarea[1]]
                            )
                            finaltrack_cell_maxETH20dbz[itrack - 1, nc] = np.nanmax(
                                echotop20[0, cellarea[0], cellarea[1]]
                            )
                            finaltrack_cell_maxETH30dbz[itrack - 1, nc] = np.nanmax(
                                echotop30[0, cellarea[0], cellarea[1]]
                            )
                            finaltrack_cell_maxETH40dbz[itrack - 1, nc] = np.nanmax(
                                echotop40[0, cellarea[0], cellarea[1]]
                            )
                            finaltrack_cell_maxETH50dbz[itrack - 1, nc] = np.nanmax(
                            ...
                            finaltrack_status[itrack - 1, nc] = np.copy(
                                trackstatus[0, nf, cloudindex]
                            )
                            finaltrack_mergenumber[itrack - 1, nc] = np.copy(
                                trackmerge[0, nf, cloudindex]
                            )
                            finaltrack_splitnumber[itrack - 1, nc] = np.copy(
                                tracksplit[0, nf, cloudindex]
                            )
                            finaltrack_trackinterruptions[itrack - 1] = np.copy(
                            ...
                            finaltrack_cell_rangeflag[itrack - 1, nc] = np.min(
