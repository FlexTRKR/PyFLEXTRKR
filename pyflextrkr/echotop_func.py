...
def echotop_height_fast(dbz3d, height, z_dimname, shape_2d, dbz_thresh, gap, min_thick):
    ...
    # last_cloud_z: last level index where a cloud was observed (-1 = none yet)
    last_cloud_z = np.full((ny, nx), -1, dtype=np.intp)
    # passed_gap: True once the first-layer gap has been crossed for a column
    passed_gap = np.zeros((ny, nx), dtype=bool)
    # first_layer_top_idx: level index of the current first-layer top (-1 = none)
    first_layer_top_idx = np.full((ny, nx), -1, dtype=np.intp)

    for k in range(nz):
        cloud_at_k = cmask[k]  # bool [ny, nx]

        # A gap too large exists when:
        #   - we've seen cloud before (last_cloud_z >= 0), AND
        #   - there is cloud at level k, AND
        #   - the spacing from the last cloud level exceeds `gap`
        cloud_seen = last_cloud_z >= 0
        gap_too_large = cloud_seen & cloud_at_k & ((k - last_cloud_z) > gap)
        passed_gap |= gap_too_large

        # A level is still in the first layer if it has cloud AND we haven't
        # crossed a layer-breaking gap yet.
        in_first_layer = cloud_at_k & ~passed_gap

        # Update the running first-layer top index
        first_layer_top_idx = np.where(in_first_layer, k, first_layer_top_idx)

        # Advance the last-seen cloud level tracker
        last_cloud_z = np.where(cloud_at_k, k, last_cloud_z)
