...
def trackstats_wrf(datasource, datadescription, pixel_radius, geolimits, areathresh, cloudtb_threshs, absolutetb_threshs, startdate, enddate, timegap, cloudid_filebase, tracking_inpath, stats_path, track_version, tracknumbers_version, tracknumbers_filebase, lengthrange=[2,120]):
    # Inputs:
    ...
    for nf in range(0,nfiles-1):
        ...
        if np.nanmax(file_tracknumbers) > 0:
            ...
            #print((chartostring(cloudidfiles_list[nf])))
            print(cloudidfiles_list[nf])
            
            # Load cloudid file
            ##cloudid_file = tracking_inpath + ''.join(cloudidfiles[nf])
            ##import pdb; pdb.set_trace()
            #cloudid_file = tracking_inpath + str(chartostring(cloudidfiles[nf]))
            cloudid_file = tracking_inpath + cloudidfiles_list[nf]
            ...
            for itrack in uniquetracknumbers:
                ...
                if len(cloudnumber) == 1: # Should only be one cloud number. In mergers and split, the associated clouds should be listed in the file_splittracknumbers and file_mergetracknumbers
                    ...
                    #if itrack == 4771:
                        #print('itrack = 4771 and coldarea = ', coldarea)
                    ...
                    #if itrack == 4771:
                        #print('itrack = 4771 and coldarea = ', corecoldarea)
                    ...
                    #if itrack == 4771:
                        #print('np.shape(lengthindex)[1]: ',np.shape(lengthindex)[1])
